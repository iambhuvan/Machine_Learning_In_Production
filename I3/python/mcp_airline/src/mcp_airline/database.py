...
class AirlineDatabase:
    ...
    def __init__(self, db_path: Path | str):
        ...
        self._security_state_path = self._db_path.with_name(
            f"{self._db_path.stem}_security_state.json"
        )
        self._data = self._load_data()
        self._security_state = self._new_security_state()
        self._load_security_state()

    def _new_security_state(self) -> dict:
        """Create transient trusted state for guarantees and receipts."""
        return {
            "pending_actions": {},
            "receipts": {},
            "compensated_reservations": {},
        }

    def _load_security_state(self) -> None:
        """Load the persistent subset of trusted state, if present."""
        if not self._security_state_path.exists():
            return

        try:
            with self._security_state_path.open("r", encoding="utf-8") as f:
                persisted = json.load(f)
        except Exception as e:
            logger.warning("Failed to load security state from %s: %s", self._security_state_path, e)
            return

        compensated = persisted.get("compensated_reservations", {})
        if isinstance(compensated, dict):
            self._security_state["compensated_reservations"] = compensated

    def persist_security_state(self) -> None:
        """Persist the durable subset of trusted state across server restarts."""
        payload = {
            "compensated_reservations": self._security_state["compensated_reservations"],
        }
        with self._security_state_path.open("w", encoding="utf-8") as f:
            json.dump(payload, f, indent=2, sort_keys=True)

    def clear_security_state(self) -> None:
        """Delete any persisted trusted state."""
        if self._security_state_path.exists():
            self._security_state_path.unlink()
    ...
    def get_security_state(self) -> dict:
        """Return the trusted runtime state used for guarantees."""
