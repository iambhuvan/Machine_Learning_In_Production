...
def register_web_routes(mcp: FastMCP, database: AirlineDatabase) -> None:
    ...
    def _is_authorized(request: Request, user_id: str) -> bool:
        """Treat the user id as a shared secret for the simple profile UI."""

        return request.headers.get("X-User-ID") == user_id
    ...
    async def get_profile(request: Request) -> JSONResponse:
        ...
        if not user_id:
            return JSONResponse({"error": "user_id is required"}, status_code=400)
        if not _is_authorized(request, user_id):
