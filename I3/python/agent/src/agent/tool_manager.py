...
def _is_hidden_tool(tool_name: str) -> bool:
    """Return whether a tool should be hidden from the model."""

    return tool_name in HIDDEN_TOOL_NAMES or tool_name.startswith("_trusted_")
...
class ToolManager:
        ...
        try:
            ...
            for tool_def in converted_tools:
                ...
                if tool_name in self.tool_to_server_map:
                    ...
                    if not _is_hidden_tool(tool_name):
                        self.tools.append(tool_def)
                        self.exposed_tool_names.add(tool_name)
            ...
            tool_names = [
                ...
                if not _is_hidden_tool(t['function']['name'])
            ]
    ...
    def execute_hidden_tool(self, tool_name: str, input_data: Dict[str, Any]) -> str:
        """
        Execute a hidden trusted tool that is unavailable to the model.

        Args:
            tool_name: Hidden tool name
            input_data: Tool arguments

        Returns:
            Tool execution result as a string
        """
        if tool_name in self.exposed_tool_names:
            raise Exception(f"Tool {tool_name} is not hidden")
        ...
        try:
            return mcp_server.call_tool(tool_name, input_data)
        except Exception as error:
