...
from zerver.models import Message, Realm, RealmAuditLog, RealmExport, Stream, UserMessage, UserProfile
...
class DeferredWorker(QueueProcessingWorker):
    ...
    def consume(self, event: dict[str, Any]) -> None:
        ...
        if event["type"] == "mark_stream_messages_as_read":
        ...
        elif event["type"] == "check_topic_drift":
            # Topic drift detection - Tier 2 AI confirmation
            from zerver.lib.topic_improver import confirm_drift_and_suggest, get_topic_context_messages
            ...
            logger.info(
                "Processing topic drift check for message %s in topic '%s'",
                message_id,
                topic_name,
            )

            # Window-Based Validation: Fetch last 5 messages
            ...
            if len(messages) < 5:
                # We need a full window of 5 messages before we judge drift.
                # This ensures we don't jump the gun on a new topic.
                logger.info(
                    "Insufficient context (%d/5 messages). Waiting for full window.",
                    len(messages)
                )
                return

            # Re-run Tier 1 Check on the window
            from zerver.lib.topic_drift import extract_topic_keywords, detect_drift_in_content
            topic_keywords = extract_topic_keywords(topic_name)
            
            drifted_count = 0
            debug_drift_info = []
            for msg in messages:
                is_drifted = detect_drift_in_content(msg['content'], topic_keywords)
                if is_drifted:
                    drifted_count += 1
                debug_drift_info.append(f"{'DRIFT' if is_drifted else 'OK'}: {msg['content'][:15]}...")
            
            logger.info("Drift Window Analysis: %s", debug_drift_info)
            
            # Threshold: If fewer than 3 of the last 5 messages are drifted, abort.
            if drifted_count < 3:
                logger.info(
                    "Drift threshold not met (%d/5 drifted). Waiting for more evidence.", 
                    drifted_count
                )
                return

            logger.info("Drift confirmed by window check (%d/5). Proceeding to Tier 2.", drifted_count)

            # Tier 2: AI confirmation
            is_drifted, suggestion = confirm_drift_and_suggest(messages, topic_name)

            if is_drifted and suggestion:
                # Send DM to the message sender with the suggestion
                sender = UserProfile.objects.get(id=sender_id)
                realm = Realm.objects.get(id=realm_id)
                notification_bot = get_system_bot(settings.NOTIFICATION_BOT, realm.id)

                # Construct topic move link
                # Format: #narrow/stream/{stream_id}/topic/{topic}
                topic_link = f"#narrow/channel/{stream_id}/topic/{topic_name}"

                with override_language(sender.default_language):
                    content = _(
                        "It looks like the conversation in this topic has drifted. "
                        "I suggest renaming it to: **{suggestion}**.\n\n"
                        "[Move Topic]({topic_link})"
                    ).format(
                        suggestion=suggestion,
                        topic_link=topic_link,
                    )

                # Auto-Rename Logic (Requirement: Change without user interaction)
                from zerver.models import Message, Stream
                try:
                    # Fetch the actual message object to use as the target for the edit
                    target_message = Message.objects.get(id=message_id)
                    
                    # We need the stream object to construct the edit request
                    stream = Stream.objects.get(id=target_message.recipient.type_id)
                    
                    # Construct the edit request to rename the topic
                    # We must populate all required fields for StreamMessageEditRequest
                    edit_request = StreamMessageEditRequest(
                        ...
                        target_topic_name=suggestion,
                        target_stream=stream,
                        orig_content=target_message.content,
                    ...
                    logger.info(
                        "AUTO-RENAMED topic '%s' to '%s' due to detected drift",
                        topic_name,
                        suggestion,
                    )
                except Exception as e:
                    logger.error("Failed to auto-rename topic: %s", e)
            else:
                logger.info("No drift confirmed for topic '%s'", topic_name)
