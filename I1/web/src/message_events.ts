    ...
    for (const event of events) {
        ...
        if (!topic_edited && !stream_changed) {
            ...
            if (
                ...
                (selection_changed_topic || !current_filter.has_operator("search"))
            ) {
                let new_filter = current_filter;
                let redirect_needed = false;
                ...
                if (redirect_needed) {
                    console.log("Auto-Rename: Switching narrow from", orig_topic, "to", new_topic);

                    // 1. Aggressively clear caches for the OLD filter to prevent "No results"
                    message_list_data_cache.remove(current_filter);

                    // 2. Force the browser URL to update immediately
                    const terms = new_filter.terms();
                    const new_hash = hash_util.search_terms_to_hash(terms);
                    browser_history.update(new_hash);

                    // 3. Force a complete re-render of the message view
                    // We use a small timeout to let the browser processing catch up
                    setTimeout(() => {
                        message_view.show(terms, {
                            trigger: "auto_rename_redirect",
                            then_select_id: current_selected_id,
                            force_rerender: true,      // Force the list to rebuild
                            force_close: true,         // Close any overlays
                        });

                        // 4. Ensure the compose box matches the new topic
                        compose_fade.update_message_list();
                        if (compose_state.topic() === orig_topic) {
                            compose_state.topic(new_topic);
                        }
                    }, 0);
                }
