...
| | **DVC** | **Roar** |
|---|---|---|
| **Philosophy** | Declarative — you define the pipeline in `dvc.yaml` | Observational — infers the DAG from file I/O at runtime |
| **Config required** | Yes — `dvc.yaml` (stages, deps, params, outs) | None — just prefix commands with `roar run` |
| **How is the DAG defined?** | Explicitly declared by the user | Auto-inferred from actual file reads/writes |
| **What happens on re-run?** | Skips cached stages; only re-runs what changed | Records a new job entry with updated hashes |
| **How are artifacts versioned?** | `.dvc` pointer files in Git + content in a remote cache | Content hashes (blake3) in local SQLite + GLaaS |
| **Reproducibility mechanism** | `dvc repro` + `dvc.lock`; restore with `dvc exp apply` | `roar reproduce <hash>` replays provenance chain |
| **Collaboration model** | `dvc.yaml` in Git + shared remote via `dvc push/pull` | Lineage published to GLaaS; teammates search by hash |
| **Platform support** | macOS, Linux, Windows (native) | macOS, Linux; Windows requires WSL2 |
